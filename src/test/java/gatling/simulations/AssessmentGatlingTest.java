 ...
 */
public class AssessmentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all assessments").get("/api/assessments").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new assessment")
                        ...
                        .body(
                            StringBody(
                                ...
                                "\"title\": \"SAMPLE_TEXT\"" +
                                ", \"description\": null" +
                                ", \"languageFramework\": \"SAMPLE_TEXT\"" +
                                ", \"difficultyLevel\": \"SAMPLE_TEXT\"" +
                                ", \"timeLimit\": 0" +
                                ", \"type\": \"SAMPLE_TEXT\"" +
                                ", \"validationCriteria\": \"SAMPLE_TEXT\"" +
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created assessment").get("${new_assessment_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created assessment").delete("${new_assessment_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Assessment entity").exec(scn);
