 ...
 */
public class SubmissionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all submissions").get("/api/submissions").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new submission")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"screenshots\": null" +
                                ", \"videoExplanation\": \"SAMPLE_TEXT\"" +
                                ", \"textDescription\": \"SAMPLE_TEXT\"" +
                                ", \"feedback\": null" +
                                ", \"pointsScored\": 0" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_submission_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created submission").get("${new_submission_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created submission").delete("${new_submission_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Submission entity").exec(scn);
