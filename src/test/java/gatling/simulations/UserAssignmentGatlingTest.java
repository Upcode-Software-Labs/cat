...
/**
 * Performance test for the UserAssessment entity.
 */
public class UserAssignmentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all userAssessments").get("/api/user-assessments").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new userAssessment")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_userAssessment_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created userAssessment").get("${new_userAssessment_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created userAssessment").delete("${new_userAssessment_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UserAssessment entity").exec(scn);
