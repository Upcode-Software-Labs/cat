 ...
 */
public class ValidationRuleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all validationRules").get("/api/validation-rules").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new validationRule")
                        .post("/api/validation-rules")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"description\": null" + ", \"validationScript\": null" + ", \"ruleType\": \"SAMPLE_TEXT\"" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_validationRule_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created validationRule").get("${new_validationRule_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created validationRule").delete("${new_validationRule_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ValidationRule entity").exec(scn);
