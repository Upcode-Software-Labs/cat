...
/**
 * Performance test for the SubmissionResult entity.
 */
public class SubmissionResultGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all submissionResults").get("/api/submission-results").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new submissionResult")
                        .post("/api/submission-results")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"totalPoints\": 0" + ", \"detailedResults\": null" + ", \"feedback\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_submissionResult_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created submissionResult").get("${new_submissionResult_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created submissionResult").delete("${new_submissionResult_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SubmissionResult entity").exec(scn);
