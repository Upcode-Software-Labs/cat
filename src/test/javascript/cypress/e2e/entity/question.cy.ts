...
describe('Question e2e test', () => {
  ...
  const questionSample = { questionText: 'Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci50eHQ=' };
  ...
  describe('new Question page', () => {
    ...
    it('should create an instance of Question', () => {
      cy.get(`[data-cy="questionText"]`).type('../fake-data/blob/hipster.txt');
      cy.get(`[data-cy="questionText"]`).invoke('val').should('match', new RegExp('../fake-data/blob/hipster.txt'));

      cy.get(`[data-cy="codeSnippet"]`).type('../fake-data/blob/hipster.txt');
      cy.get(`[data-cy="codeSnippet"]`).invoke('val').should('match', new RegExp('../fake-data/blob/hipster.txt'));

      cy.get(`[data-cy="resources"]`).type('freely loftily causal');
      cy.get(`[data-cy="resources"]`).should('have.value', 'freely loftily causal');

      cy.get(`[data-cy="points"]`).type('15400');
      cy.get(`[data-cy="points"]`).should('have.value', '15400');
