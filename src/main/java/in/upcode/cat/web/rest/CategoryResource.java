...
import in.upcode.cat.service.CategoryService;
...
public class CategoryResource {
     ...
     *
     * @param assignmentType the categoryDTO to create.
    ...
    public ResponseEntity<CategoryDTO> createCategory(
        @RequestPart("assignmentType") String assignmentType,
        @RequestPart("image") MultipartFile image
    ) throws URISyntaxException {
        //log.debug("REST request to save Category : {}", categoryDTO);
        CategoryDTO categoryDTO = new CategoryDTO();
        categoryDTO.setAssignmentType(assignmentType);
        if (categoryDTO.getId() != null) {
            throw new BadRequestAlertException("A new category cannot already have an ID", ENTITY_NAME, "idexists");
        }
        try {
            byte[] imageBytes = image.getBytes();
            categoryDTO.setImage(imageBytes);
        } catch (IOException e) {
